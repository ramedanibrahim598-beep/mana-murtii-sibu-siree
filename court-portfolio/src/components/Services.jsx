...
export default function Services() {
  return (
    <section id="services" className="py-20 px-6 section-primary">
      <div className="max-w-6xl mx-auto text-center">
        <span className="text-yellow-600 font-semibold uppercase tracking-widest text-sm">
          Maal Dhiyeessina
        </span>
        <h2 className="text-3xl md:text-4xl font-bold mt-2 mb-12 text-main">
          Tajaajila Mana Murtii
        </h2>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((s) => (
            <div key={s.title} className="service-card rounded-xl p-6 text-left border hover:shadow-lg hover:border-yellow-300 transition-all duration-200">
              <div className="text-4xl mb-4">{s.icon}</div>
              <h3 className="text-lg font-bold mb-2 text-main">{s.title}</h3>
